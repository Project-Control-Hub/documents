---
문서명: Project Control Hub 아키텍처 정의서
버전: v4.0
작성일: 2026-03-21
최종수정일: 2026-03-22
작성자: 팀
...
Project Control Hub의 전체 시스템 아키텍처를 정의한다. 이슈 트래킹, 워크플로우 엔진, 스프린트 관리, 대시보드, REST API를 포함하는 통합 협업 플랫폼이다.

제품 기능 정의서(`Project_Control_Hub.md` v3.4) 기준으로 정의된 핵심 기능 영역인 이슈 관리, 프로젝트 관리, 워크플로우, JQL 검색, 알림, 외부 연동, 자동화, 권한 구조를 모두 지원하는 아키텍처를 구성한다.
...
| CloudWatch | 로그 수집 및 알람 | 로그 그룹: /pch-app/api |
...
| 검색 | JQL Parser (Custom) | JQL(Jira Query Language) 지원 |
...
sequenceDiagram
    ...
    participant PCHAPI as PCH API
    participant Workflow as 워크플로우 엔진
    participant Slack as Slack

    Dev->>GitHub: PR 생성 (브랜치: feature/PROJ-142-fix)
    GitHub->>PCHAPI: Webhook POST /api/v1/webhook/github (PR opened)
    PCHAPI->>PCHAPI: HMAC-SHA256 서명 검증
    PCHAPI->>Workflow: StatusChangeCommand (PROJ-142, "Code Review")
    Workflow->>Workflow: 전환 규칙 검증 (In Progress → Code Review)
    Workflow->>PCHAPI: 상태 전환 완료
    PCHAPI->>Slack: 코드 리뷰 요청 알림 (#code-review)

    Dev->>GitHub: PR Merge (main 브랜치)
    GitHub->>PCHAPI: Webhook POST (PR merged)
    PCHAPI->>Workflow: StatusChangeCommand (PROJ-142, "Done")
    Workflow->>PCHAPI: 상태 전환 완료
    PCHAPI->>Slack: Done 전환 알림 (#proj-a-alerts)
```
...
final dio = Dio(BaseOptions(
  baseUrl: 'https://api.pch.example.com',
